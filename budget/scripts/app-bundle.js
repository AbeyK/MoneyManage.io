...
define('about/about',['exports', 'aurelia-framework', 'aurelia-router', '../services/user'], function (exports, _aureliaFramework, _aureliaRouter, _user) {
  ...
  var about = exports.about = (_dec = (0, _aureliaFramework.inject)(_aureliaRouter.Router, _user.User), _dec(_class = function about() {
...
define('login/login',['exports', 'aurelia-framework', 'aurelia-router', '../services/user'], function (exports, _aureliaFramework, _aureliaRouter, _user) {
    'use strict';
    ...
    var login = exports.login = (_dec = (0, _aureliaFramework.inject)(_aureliaRouter.Router, _user.User), _dec(_class = function () {
        ...
        login.prototype.loginClick = function loginClick() {
            this.user.personalInfo.showNavbar = true;
            this.router.navigate('#/personalinfo');
        };
    ...
    var results = exports.results = (_dec = (0, _aureliaFramework.inject)(_aureliaRouter.Router, _user.User, _chart.Chart, _constants.Constants, _calculateExpenses.calculateExpenses, _calculatePercentages.calculatePercentages, _calculateRecommended.calculateRecommended), _dec(_class = function () {
        ...
        results.prototype.formatModal = function formatModal() {
            ...
            for (var i = 0; i < this.homeChanges.length; i++) {
                ...
                if (val < 0) this.modelHomeChanges.push("-$" + Math.abs(val).toFixed(2));else this.modelHomeChanges.push("$" + Math.abs(val).toFixed(2));
            }

            for (var i = 0; i < this.carChanges.length; i++) {
                var val = parseFloat(this.user.recommend.carChanges[this.carChanges[i]]);
                if (val < 0) this.modelCarChanges.push("-$" + Math.abs(val).toFixed(2));else this.modelCarChanges.push("$" + Math.abs(val).toFixed(2));
            }

            for (var i = 0; i < this.healthChanges.length; i++) {
                var val = parseFloat(this.user.recommend.healthChanges[this.healthChanges[i]]);
                if (val < 0) this.modelHealthChanges.push("-$" + Math.abs(val).toFixed(2));else this.modelHealthChanges.push("$" + Math.abs(val).toFixed(2));
            }
            ...
            for (var i = 0; i < this.discretionaryChanges.length; i++) {
                ...
                if (val < 0) this.modelDiscretionaryChanges.push("-$" + Math.abs(val).toFixed(2));else this.modelDiscretionaryChanges.push("$" + Math.abs(val).toFixed(2));
            }
                ...
                for (var i = 0; i < this.constants.HomeExpenses.length; i++) {
                    ...
                    if (!this.user.recommend[expenseName + 'lock']) ;else {
                        if (expenseName == 'phone') {
                            ...
                            if (this.user.recommend['cable'] == 0) adjusted = this.user.expenses.homeExpenseConstants.Streaming;else adjusted = this.user.recommend[expenseName];
...
define('text!results/results.html', ['module'], function(module) { module.exports = "<template><div id=\"results\"><br><br><compose view-model=\"./compose/compose-chart\"></compose><compose view-model=\"./compose/compose-recommendations\"></compose><compose view-model=\"./compose/compose-table\"></compose><hr><button class=\"btn btn-secondary btn-lg\" style=\"float:left;width:10%\" click.delegate=\"back()\"><span class=\"glyphicon glyphicon-arrow-left\"></span></button> <button class=\"btn btn-lg\" style=\"float:center;width:20%;background-color:#dff0d8\" click.delegate=\"getChartData()\">Make New Chart</button> <button class=\"btn btn-primary btn-lg\" style=\"float:right;width:10%\" click.delegate=\"home()\">Finish</button><br><br><br></div></template>"; });
...
define('text!expenses/compose/compose-home-expenses.html', ['module'], function(module) { module.exports = "<template><br><div class=\"${click ? 'none' : 'box-shadow--6dp'}\" style=\"border-radius:0\"><div><h4 class=\"panel-title\"><div style=\"float:right;padding:5px;width:30%;text-align:right\">Total: $${user.expenses.totalHomeExpense}</div><a data-toggle=\"collapse\" data-parent=\"#accordion\" href=\"#homeExpenseCollapse\" click.delegate=\"clicked()\" style=\"float:left;padding:5px;width:30%\"><span show.bind=\"click\" class=\"glyphicon glyphicon-chevron-right\"></span> <span show.bind=\"!click\" class=\"glyphicon glyphicon-chevron-down\"></span> </a><a data-toggle=\"collapse\" data-parent=\"#accordion\" href=\"#homeExpenseCollapse\" click.delegate=\"clicked()\" style=\"font-size:24px\"><center>Home <span id=\"expensesTooltip\" title=\"\" class=\"glyphicon glyphicon-question-sign\"></span></center></a></h4></div><div id=\"homeExpenseCollapse\" class=\"panel-collapse collapse in\"><div class=\"panel-body\"><div repeat.for=\"home of constants.HomeExpenses\" class=\"form-group col-md-6\"><label style=\"padding-left:5%\">${home.title}</label><div class=\"input-group mb-2 mr-sm-2 mb-sm-0 expensesInput\"><div class=\"input-group-addon\">$</div><input type=\"text\" value.bind=\"user.expenses[home.value]\" change.delegate=\"calculateExpenses.homeExpenses()\" class=\"form-control\" id=\"${home.value}\"><div class=\"input-group-btn\"><button type=\"button\" class=\"btn ${user.expenses[home.value + 'lock'] ? 'btn-default' : 'btn-primary'}\" click.delegate=\"lockStateChange(home.value)\"><i class=\"fa fa-unlock\" show.bind=\"!user.expenses[home.value + 'lock']\"></i> <i class=\"fa fa-lock\" show.bind=\"user.expenses[home.value + 'lock']\"></i></button></div></div><br></div></div></div></div></template>"; });
define('text!results/compose/compose-chart.html', ['module'], function(module) { module.exports = "<template><div class=\"box-shadow--6dp\" show.bind=\"user.results.showBudget\" style=\"float:left;margin-right:4%;width:48%\" id=\"resultsContainerSimple\"></div><div class=\"box-shadow--6dp\" show.bind=\"user.results.showAdvanced\" style=\"float:left;margin-right:4%;width:48%\" id=\"resultsContainerAdvanced\"></div><div class=\"box-shadow--6dp\" show.bind=\"user.results.showGoalsChart\" style=\"float:left;margin-right:4%;width:48%\" id=\"fiveYearGoalsContainer\"></div><div class=\"box-shadow--6dp\" show.bind=\"user.results.showExpenses\" style=\"float:left;margin-right:4%;width:48%\" id=\"fiveYearExpensesContainer\"></div><div class=\"box-shadow--6dp\" show.bind=\"!user.results.showAdvancedRecommended\" style=\"float:left;width:48%\" id=\"recommendedContainer\"></div><div class=\"box-shadow--6dp\" show.bind=\"user.results.showAdvancedRecommended\" style=\"float:left;width:48%\" id=\"recommendedContainerAdvanced\"></div><br style=\"clear:both\"><br style=\"clear:both\"><br style=\"clear:both\"><div class=\"form-group col-md-6\" style=\"width:48%;margin-right:4%\"><label for=\"race\">Chart Options</label><select class=\"form-control\" value.bind=\"option\" change.delegate=\"showChart(option)\"><option repeat.for=\"option of chartOptions\">${option.text}</option></select></div><div class=\"form-group col-md-6\" style=\"width:48%\"><label for=\"race\">Recommended Chart Options</label><select class=\"form-control\" value.bind=\"option\" change.delegate=\"showRecommendedChart(option)\"><option>Recommended Simple Budget</option><option>Recommended Advanced Budget</option></select></div><br><br><br><br></template>"; });
define('text!results/compose/compose-recommendations.html', ['module'], function(module) { module.exports = "<template><br style=\"clear:both\"><div class=\"${user.recommend.messageStyle}\"><b>${user.recommend.message}</b><br><br><button class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#changes\">See Recommended Changes</button><div class=\"modal fade\" id=\"changes\" role=\"dialog\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\">&times;</button><h4 class=\"modal-title\"><b>Your Changes</b></h4></div><div class=\"modal-body\"><div class=\"row\"><div class=\"col-md-6\"><h5><b>Home Changes</b></h5><div repeat.for=\"homeChange of homeChanges\" class=\"${user.recommend.homeChanges[homeChange] < 0 ? 'decreasing' : 'adding'}\">${homeChange.charAt(0).toUpperCase() + homeChange.slice(1)}: ${modelHomeChanges[$index]}</div></div><div class=\"col-md-6\"><h5><b>Transportation Changes</b></h5><div repeat.for=\"carChange of carChanges\" class=\"${user.recommend.carChanges[carChange] < 0 ? 'decreasing' : 'adding'}\">${carChange.charAt(0).toUpperCase() + carChange.slice(1)}: ${modelCarChanges[$index]}</div></div></div><hr><div class=\"row\"><div class=\"col-md-6\"><h5><b>Health Changes</b></h5><div repeat.for=\"healthChange of healthChanges\" class=\"${user.recommend.healthChanges[healthChange] < 0 ? 'decreasing' : 'adding'}\">${healthChange.charAt(0).toUpperCase() + healthChange.slice(1)}: ${modelHealthChanges[$index]}</div></div><div class=\"col-md-6\"><h5><b>Discretionary Changes</b></h5><div repeat.for=\"discretionaryChange of discretionaryChanges\" class=\"${user.recommend.discretionaryChanges[discretionaryChange] < 0 ? 'decreasing' : 'adding'}\">${discretionaryChange.charAt(0).toUpperCase() + discretionaryChange.slice(1)}: ${modelDiscretionaryChanges[$index]}</div><h5 class=\"${user.recommend.expensesChange < 0 ? 'decreasing' : 'adding'}\"><b>Total Expense Change: ${modelExpenseChange}</b></h5><h5 style=\"color:#3c763d\"><b>Savings Change: $${user.recommend.savingsChange.toFixed(2)}</b></h5></div></div></div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Close</button></div></div></div></div></div></template>"; });
define('text!results/compose/compose-table.html', ['module'], function(module) { module.exports = "<template><hr style=\"clear:both\"><div style=\"width:100%;margin:0 auto\" class=\"table-outter\"><table class=\"table table-bordered box-shadow--6dp\"><thead class=\"bg-primary\"><tr style=\"font-size:20px\"><th style=\"text-align:center\" repeat.for=\"expense of user.results.recommendedResults.length\">${user.results.recommendedResults[expense][0]}</th><th style=\"text-align:center\">Yearly Savings</th></tr></thead><tbody><tr><td repeat.for=\"amount of user.results.recommendedResults.length\"><div style=\"text-align:center\"><strong>$ ${user.expenses['total' + user.results.recommendedResults[amount][0] + 'Expense']}</strong></div><hr><div style=\"height:300px;overflow-y:scroll\"><div repeat.for=\"expense of constants[user.results.recommendedResults[amount][0] + 'Expenses']\" class=\"form-group\"><label>${expense.title}</label><div class=\"input-group mb-2 mr-sm-2 mb-sm-0\"><div class=\"input-group-addon\">$</div><input type=\"text\" value.bind=\"user.expenses[expense.value]\" disabled.bind=\"!user.expenses[expense.value + 'lock']\" change.delegate=\"checkValue(user.expenses, user.expenses[expense.value], expense, user.results.recommendedResults[amount][0])\" class=\"form-control ${user.expenses[expense.value + 'check'] ? 'alert-success' : 'alert-danger'}\"></div><br></div></div></td><td><div style=\"text-align:center\"><strong>$ ${user.personalInfo.savingsPerMonth * 12}</strong></div><hr><div class=\"form-group\"><label for=\"privateSchool\">Savings Per Month</label><div class=\"input-group mb-2 mr-sm-2 mb-sm-0\"><div class=\"input-group-addon\">$</div><input type=\"text\" value.bind=\"user.personalInfo.savingsPerMonth\" class=\"form-control\"></div><br></div><div show.bind=\"user.results.showGoals\" style=\"height:200px;overflow-y:scroll\"><div repeat.for=\"wish of constants.wishes\"><div show.bind=\"user.personalInfo[wish.check]\"><div class=\"form-group\"><label for=\"privateSchool\">Amount for ${wish.title}</label><div class=\"input-group mb-2 mr-sm-2 mb-sm-0\"><div class=\"input-group-addon\">$</div><input type=\"text\" value.bind=\"user.personalInfo[wish.value]\" class=\"form-control ${user.results[wish.value + 'MetGoal'] ? 'alert-success' : 'alert-danger'}\"></div></div><br></div></div></div></td></tr></tbody></table></div><div style=\"width:45%;margin:0 auto;text-align:center\" class=\"alert alert-success\" role=\"alert\"><strong>This means your expense is lower than average or you meet your goal</strong></div><div style=\"width:45%;margin:0 auto;text-align:center\" class=\"alert alert-danger\" role=\"alert\"><strong>This means your expense is higher than average or you didn't meet your goal</strong></div></template>"; });
