...
define('utilities/calculateExpenses',['exports', 'aurelia-framework', '../services/user', '../services/expensesConstants'], function (exports, _aureliaFramework, _user, _expensesConstants) {
    ...
    var calculateExpenses = exports.calculateExpenses = (_dec = (0, _aureliaFramework.inject)(_user.User, _expensesConstants.ExpensesConstants), _dec(_class = function () {
        ...
        calculateExpenses.prototype.makeChartArry = function makeChartArry() {
            for (var i = 1; i < 5; i++) {
                var tempHomeTotal = parseInt(this.user.expenses.mortgage) * Math.pow(1.01, i) + parseInt(this.user.expenses.propertyTax) + parseInt(this.user.expenses.phone) * Math.pow(1 - 8.60, i) + parseInt(this.user.expenses.internet) * Math.pow(1 - .004, i) + parseInt(this.user.expenses.cable) * Math.pow(1.028, i) + parseInt(this.user.expenses.netfix) + parseInt(this.user.expenses.groceries) * Math.pow(1 - 1.002, i) + parseInt(this.user.expenses.utilities * Math.pow(1.046, i)) + parseInt(this.user.expenses.homeMaintenance) + parseInt(this.user.expenses.clothes) * Math.pow(1 - 0.009);
...
define('text!results/compose/compose-chart.html', ['module'], function(module) { module.exports = "<template><div class=\"col-md-4\" style=\"margin-left:25%\"><div class=\"btn-group\" data-toggle=\"buttons\"><label repeat.for=\"option of someOptions\" class=\"btn btn-primary\" click.delegate=\"test(option.text)\"><input type=\"radio\" autocomplete=\"off\" model.bind=\"option\" checked.bind=\"$parent.selectedOptions\"> ${option.text}</label></div></div><div class=\"col-md-4\"><div class=\"btn-group\" click.delegate=\"checkAdvancedRecommended()\" data-toggle=\"buttons\"><label class=\"btn ${!user.results.showAdvancedRecommended ? 'active btn-primary' : 'btn-secondary'}\"><input type=\"radio\">Recommended Simple Budget</label><label class=\"btn ${user.results.showAdvancedRecommended ? 'active btn-primary' : 'btn-secondary'}\"><input type=\"radio\">Recommended Advanced Budget</label></div></div><div show.bind=\"user.results.showExpenses\" style=\"float:left;margin-left:20%\" id=\"fiveYearContainer\"></div><div show.bind=\"user.results.showBudget\" style=\"float:left;margin-left:20%\" id=\"resultsContainer\"></div><div show.bind=\"user.results.showAdvanced\" style=\"float:left;margin-left:20%\" id=\"resultsContainerAdvanced\"></div><div show.bind=\"!user.results.showAdvancedRecommended\" style=\"float:left\" id=\"recommendedContainer\"></div><div show.bind=\"user.results.showAdvancedRecommended\" style=\"float:left\" id=\"recommendedContainerAdvanced\"></div></template>"; });
