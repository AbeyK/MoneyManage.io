    ...
    var expenses = exports.expenses = (_dec = (0, _aureliaFramework.inject)(_aureliaRouter.Router, _user.User, _constants.Constants, _calculateExpenses.calculateExpenses, _expensesConstants.ExpensesConstants), _dec(_class = function () {
        ...
        expenses.prototype.attached = function attached() {
            ...
            $('#expensesTooltip').tooltip({
                content: "Enter all expenses as monthly amounts unless stated otherwise.<br>" + "Lock values you don't want changed."
            });
        };
    ...
    var calculateRecommended = exports.calculateRecommended = (_dec = (0, _aureliaFramework.inject)(_user.User, _constants.Constants, _expensesConstants.ExpensesConstants), _dec(_class = function () {
        ...
        calculateRecommended.prototype.getRecommendedTotals = function getRecommendedTotals() {
            ...
            for (var i = 0; i < this.constants.CarExpenses.length; i++) {
                ...
                if (!this.user.recommend[expenseName + 'lock']) ;else {
                    if (this.user.recommend[expenseName + 'check']) ;else {
                        if (expenseName == 'carPayment') adjusted = this.expensesConstants.carExpenseConstants.Payment;else if (expenseName == 'gas') adjusted = this.expensesConstants.carExpenseConstants.Gas;else if (expenseName == 'carMaintenance') adjusted = this.expensesConstants.carExpenseConstants.Maintenance;else adjusted = this.user.recommend[expenseName] * .75;
            ...
            for (var i = 0; i < this.constants.HealthExpenses.length; i++) {
                ...
                if (!this.user.recommend[expenseName + 'lock']) ;else {
                    if (this.user.recommend[expenseName + 'check']) ;else {
                        if (expenseName == 'unexpectedMedicalProblems') adjusted = this.expensesConstants.healthExpenseConstants.Emergency;else if (expenseName == 'braces') adjusted = this.expensesConstants.healthExpenseConstants.Braces;else adjusted = this.user.recommend[expenseName] * .75;
            ...
            for (var i = 0; i < this.constants.DiscretionaryExpenses.length; i++) {
                ...
                if (!this.user.recommend[expenseName + 'lock']) ;else {
                    if (this.user.recommend[expenseName + 'check']) ;else {
                        if (expenseName == 'eatingOut') adjusted = this.expensesConstants.discretionaryExpenseConstants.Eating;else if (expenseName == 'bars') adjusted = this.expensesConstants.discretionaryExpenseConstants.Club;else adjusted = this.user.recommend[expenseName] * .75;
...
define('text!expenses/expenses.html', ['module'], function(module) { module.exports = "<template><link rel=\"stylesheet\" href=\"https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css\"><div id=\"expenses\"><h2><span id=\"expensesTooltip\" title=\"\" class=\"glyphicon glyphicon-question-sign\"></span></h2></div><form submit.delegate=\"next()\"><div class=\"container\" style=\"width:60%;padding:0\"><div class=\"panel-group\" id=\"accordion\"><compose view-model=\"./compose/compose-home-expenses\"></compose><compose view-model=\"./compose/compose-car-expenses\"></compose><compose view-model=\"./compose/compose-health-expenses\"></compose><compose view-model=\"./compose/compose-discretionary-expenses\"></compose></div></div><div id=\"expenses\"><br><br><hr><button class=\"btn btn-secondary btn-lg\" style=\"float:left;width:10%\" click.delegate=\"back()\"><span class=\"glyphicon glyphicon-arrow-left\"></span></button> <button class=\"btn btn-primary btn-lg\" style=\"float:right;width:10%\" type=\"submit\" id=\"next\"><span class=\"glyphicon glyphicon-arrow-right\"></span></button></div></form><br><br><br><br><br><br></template>"; });
